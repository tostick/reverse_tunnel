...
extern void __zlog(struct zlog *zl, int priority, const char *format, ...);

#define zlog(zl, priority, fmt, args...) \
	do{\
		switch(priority) { \
			case L_EMG: \
				__zlog(zl, priority, "  EMERG | "fmt, ##args); \
				break; \
			case L_ALERT: \
				__zlog(zl, priority, "  ALERT | "fmt, ##args); \
				break; \
			case L_CRIT: \
				__zlog(zl, priority, "   CRIT | "fmt, ##args); \
				break; \
			case L_ERR: \
				__zlog(zl, priority, "  ERROR | "fmt, ##args); \
				break; \
			case L_WARN: \
				__zlog(zl, priority, "WARNING | "fmt, ##args); \
				break; \
			case L_NOTICE: \
				__zlog(zl, priority, " NOTICE | "fmt, ##args); \
				break; \
			case L_INFO: \
				__zlog(zl, priority, "   INFO | "fmt, ##args); \
				break; \
			case L_DBG: \
				__zlog(zl, priority, "  DEBUG | "fmt, ##args); \
				break; \
		} \
	}while(0)
...
*/
#define zlog_err(fmt, args...) do {\
	__zlog(NULL, L_ERR, "  ERROR | "fmt, ##args);\
}while (0)

#define zlog_warn(fmt, args...) do {\
	__zlog(NULL, L_WARN, "WARNING | "fmt, ##args);\
}while (0)

#define zlog_info(fmt, args...) do {\
	__zlog(NULL, L_INFO, "   INFO | "fmt, ##args);\
}while (0)

#define zlog_notice(fmt, args...) do {\
	__zlog(NULL, L_INFO, " NOTICE | "fmt, ##args);\
}while (0)

#define zlog_debug(fmt, args...) do {\
	__zlog(NULL, L_DBG, "  DEBUG | "fmt, ##args);\
}while (0)
